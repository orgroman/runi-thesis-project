...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("native_async_workflow.log"),
    ]
)
...
async def main():
    """Run the native async patent negation analysis workflow."""
    if len(sys.argv) < 2:
        print("Usage: python run_native_async.py <csv_path> [batch_size]")
        sys.exit(1)
    ...
    workflow_id = f"patent-negation-async-{csv_filename}-{int(datetime.now().timestamp())}"
    
    # Start the workflow
    logger.info(f"Starting native async workflow {workflow_id} for {csv_path}")
    handle = await client.start_workflow(
        PatentNegationAsyncWorkflow.run,
