The context of this project is my thesis project... here's the general idea I'm trying to research

------------------------------------------------------------------------------------
# SenseIP Project Proposal: Text Similarity When Considering Negation

## Project Proposal: Incorporating Logical Constraints in AI-Based Text Similarity Searches

### Overview

In many information retrieval tasks, we are interested in finding similar sentences while strictly enforcing logical rules between the query and retrieved entries.

For example, in the context of searching for prior art for a patent application, the sentence:

> “The invention is a machine that has 4 wheels but it is not a car.”

should **not** be considered similar to any sentence related to cars. Nevertheless, modern text similarity approaches based on sentence embedding would not respect this condition.

This situation arises frequently in patent searches and in iterative searches when the user provides feedback based on previously retrieved results.
...
One relatively simple yet promising approach is to combine negation and other logical operators within the search, similar to standard search engine interfaces and database searches.

For example, a good starting point is to augment vector similarity with logic deduced from the wording in the sentence so that sentences not meeting the logic criteria are excluded from the list of retrieved results.
...
We will look for different examples demonstrating the issue with vanilla embedding-based similarity searches.

#### III. Literature Review of Logical Operators in Search

#### IV. Proposing and Testing a New Method

#### V. Evaluating the New Method  
We will evaluate the new method over benchmarks and the examples from Milestone II.
------------------------------------------------------------------------------------
Also here's some context about datasets that's included in my project

PatentMatch: A Dataset for Matching Patent Claims & Prior Art
Julian Risch, Christoph Hewel, Nicolas Alder, Ralf Krestel
arXiv:2012.13919v1 [cs.IR] – 27 Dec 2020
Dataset Link

Abstract
PatentMatch is a large, labeled dataset designed to support supervised machine learning for prior art search. It contains over 6 million pairs of patent claims and semantically related paragraphs from cited patent documents, labeled by professional European Patent Office (EPO) examiners. Labels indicate whether a passage is novelty-destroying (“X” documents) or merely background (“A” documents). The dataset aims to facilitate research in patent passage retrieval, a task critical for assessing the novelty and inventive step of patent claims.

Background & Motivation
Patent examiners perform complex retrieval tasks to assess novelty. They often rely on previously granted patents, where matching passages are annotated in search reports. These reports help identify whether a claim is novel or not. Due to the volume and complexity of patent texts, automation through machine learning is highly desirable.

Dataset Construction
Built from EPO’s full-text XML data (since 1978) and search reports (from 2012 onward), the dataset includes:

6,259,703 claim-paragraph pairs

3.49M labeled as “X” (positive)

2.77M labeled as “A” (negative)

31,238 applications and 33,195 cited documents Balanced and simplified subsets (347k and 25k samples) are also provided for ML use.

Experiments
A baseline BERT-based classifier achieved 52–54% accuracy, showing the difficulty of the task due to legal language and semantic complexity.

Impact
PatentMatch is a valuable resource for research in legal NLP, IR, and deep learning. It enables progress in automating prior art search and improving tools used by patent professionals. The dataset is released under a Creative Commons license for open research use.

-------------------------------------------------------------------------------
Here's the documentation for OpenAI BatchAPI I think it's important
...
---------------------------------------------------------------------------------
Also some more documentation from OpenAI about structured outputs
