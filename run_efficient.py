...
from workflow_efficient import PatentNegationAnalysisEfficientWorkflow
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("efficient_workflow.log"),
    ]
)
...
async def main():
    """Run the efficient patent negation analysis workflow."""
    if len(sys.argv) < 2:
        print("Usage: python run_efficient.py <csv_path> [batch_size]")
        sys.exit(1)
    ...
    workflow_id = f"patent-negation-efficient-{csv_filename}-{int(datetime.now().timestamp())}"
    
    # Start the workflow
    logger.info(f"Starting efficient workflow {workflow_id} for {csv_path}")
    handle = await client.start_workflow(
        PatentNegationAnalysisEfficientWorkflow.run,
    ...
    try:
        ...
        if result['status'] == 'completed':
            logger.info(f"Processed {result.get('batches_submitted', 0)} batches")
